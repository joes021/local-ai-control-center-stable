...
function barStyle(percent: number | null | undefined) {
  const safePercent = percent === null || percent === undefined || Number.isNaN(percent) ? 0 : Math.max(0, Math.min(100, percent));
  return { width: `${safePercent}%` };
}

export function CompatibilityCalculatorModal({ isOpen, title, request, onClose }: Props) {
      ...
      .then((response) => {
        ...
        if (snapshot) {
          setOverrides({
            ...
            runtimePreference: snapshot.turboQuantConfig?.runtimePreference ?? "turboquant",
          });
        }
      })
      .catch((reason: unknown) => {
        if (!cancelled) {
          const message = reason instanceof Error ? reason.message : "Compatibility provera nije uspela.";
          setResult({
            status: "error",
  ...
  async function handleRecheck(nextOverrides?: CompatibilityCheckRequest["overrides"]) {
    if (!request) {
      return;
    }
    setPending(true);
    try {
      const response = await checkModelCompatibility({ ...request, overrides: nextOverrides ?? overrides });
      setPayload(response);
      setResult(null);
    } catch (reason: unknown) {
      const message = reason instanceof Error ? reason.message : "Compatibility provera nije uspela.";
  ...
  async function handleApply(action: CompatibilityAction) {
    ...
    if (action.requiresConfirmation && !window.confirm("Ova akcija menja vise vaznih podesavanja. Nastaviti?")) {
      return;
    }
    setPending(true);
    try {
      ...
      const message = reason instanceof Error ? reason.message : "Compatibility apply nije uspeo.";
      setResult({
        status: "error",
  ...
  return (
    <div className="compat-modal-overlay" onClick={onClose}>
      <div className="compat-modal" onClick={(event) => event.stopPropagation()}>
        ...
        {pending && !payload ? <div className="helper-text">Racunam kompatibilnost...</div> : null}

        {payload ? (
          <>
            <div className="compat-header">
              <span className={fitBadgeClass(payload.fitStatus)}>{payload.fitLabel}</span>
              <span className={speedBadgeClass(payload.speedStatus)}>{payload.speedLabel ?? "Slicno"}</span>
            </div>
            ...
            <section className="compat-budget-grid">
              <div className="compat-budget-card">
                ...
                <strong className="status-value">
                  {formatGiB(payload.memoryBudget?.vram.requiredGiB)} / {formatGiB(payload.memoryBudget?.vram.availableGiB)}
                </strong>
              ...
              <div className="compat-budget-card">
                ...
                <strong className="status-value">
                  {formatGiB(payload.memoryBudget?.ram.requiredGiB)} / {formatGiB(payload.memoryBudget?.ram.availableGiB)}
                </strong>
                <div className="compat-bar">
              ...
              <div className="compat-budget-card">
                ...
                <strong className="status-value">{payload.memoryBudget?.contextPressure.label}</strong>
                <div className="compat-bar">
                  <div className="compat-bar-fill compat-bar-fill-context" style={barStyle(payload.memoryBudget?.contextPressure.usagePercent)} />
                </div>
                <div className="helper-text">
            ...
            {payload.recommendations?.length ? (
              <section className="compat-section">
                <div className="section-header">
                  ...
                  <button type="button" className="secondary-button" disabled={pending} onClick={() => void handleRecheck()}>
                    Re-check
                  </button>
                </div>
                <div className="compat-recommendation-list">
                  {payload.recommendations.map((item) => (
                    <div className="compat-recommendation" key={item.id}>
                      ...
                      {item.action ? (
                        <button type="button" disabled={pending} onClick={() => void handleApply(item.action!)}>
                          {item.action.kind === "apply-package" ? "Apply package" : "Primeni"}
                        </button>
                      ) : null}
                    </div>
                  ))}
            ...
            <section className="compat-section">
              <div className="section-header">
                ...
                <button type="button" className="secondary-button" onClick={() => setAdvancedOpen((current) => !current)}>
            ...
            {advancedOpen ? (
              <section className="compat-section">
                ...
                <div className="compat-advanced-grid">
                  <label className="browser-field">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setOverrides((current) => ({ ...current, context: Number(event.target.value) || 0 }))
                      }
                    />
                  </label>
                  <label className="browser-field">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setOverrides((current) => ({ ...current, outputTokens: Number(event.target.value) || 0 }))
                      }
                    />
                  </label>
                  <label className="browser-field">
                    ...
                    <CustomSelect
                      ...
                      onChange={(value) => setOverrides((current) => ({ ...current, ctk: value }))}
                  ...
                  <label className="browser-field">
                    ...
                    <CustomSelect
                      ...
                      options={quantOptions}
                      onChange={(value) => setOverrides((current) => ({ ...current, ctv: value }))}
                  ...
                  <label className="browser-field">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setOverrides((current) => ({ ...current, ncmoe: Number(event.target.value) || 0 }))
                      }
                    />
                  </label>
                  <label className="browser-field">
                    ...
                    <CustomSelect
                      ...
                      onChange={(value) =>
                        setOverrides((current) => ({ ...current, runtimePreference: value }))
                      }
                ...
                <div className="inline-actions compact-actions">
                  <button type="button" disabled={pending} onClick={() => void handleRecheck()}>
